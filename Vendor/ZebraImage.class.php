...
class Zebra_Image
{
            ...
            if ($this->_create_from_source()) {
                ...
                if (is_array($filter)) {
                    ...
                    foreach ($filter as $arguments) // if filter exists
                    {
                        if (defined('IMG_FILTER_' . strtoupper($arguments[0]))) {
                            ...
                            if (!@call_user_func_array(
                                'imagefilter',
                                array_merge(
                                    array($target_identifier, constant('IMG_FILTER_' . strtoupper($arguments[0]))),
                            ...
                            {
                                trigger_error(
                                    'Invalid arguments used for "' . strtoupper($arguments[0]) . '" filter',
                                    E_USER_WARNING
                                );
                            }
                            ...
                            trigger_error(
                                'Filter "' . strtoupper($arguments[0]) . '" is not available',
                                E_USER_WARNING
                            );
                        }
                    }
                    ...
                    if (!@call_user_func_array(
                        'imagefilter',
                        array_merge(
                            array($target_identifier, constant('IMG_FILTER_' . strtoupper($filter))),
                            array_slice($arguments, 1)
                        )
                    )
                    ) // ...and trigger an error if the filter could not be applied
    ...
    function _create_from_source()
    {
        ...
        if (!function_exists('gd_info')) {
        ...
        } elseif (!list($this->source_width, $this->source_height, $this->source_type) = @getimagesize(
            $this->source_path
        )
        ) {
            ...
            switch ($this->source_type) {
                ...
                case IMAGETYPE_GIF:
                    ...
                    if (($this->source_transparent_color_index = imagecolortransparent(
                            $identifier
                        )) >= 0
